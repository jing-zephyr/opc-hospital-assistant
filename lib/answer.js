...
import { officialOf, KNOWN_HOSPITALS, categoryOf, isPlausibleHospitalName, stripCategorySuffix, OFFICIAL_SITES, looksLikeSearchEcho, categoryEvidenceOf, categoryByName, isTrimmedVariantOf } from './hospitals.js'
...
export function extractHospitals(items, opts = {}) {
  ...
  ;(items || []).forEach((it, rank) => {
    ...
    if (itOff && itOff.kind === 'authority') return
  ...
  for (const h of merged) {
    ...
    const official = off && off.kind === 'hospital-official' ? off.name : ''
  ...
  const trustedEntry = (h) => {
    const off = officialOf(hostOf(h.url))
    if (off && off.kind === 'authority') return false          // 主管部门/院校**不是医院**（"首都医科大学"）
...
function renderHospitalCards(hos, ctx) {
  ...
  hos.slice(0, 5).forEach((h, i) => {
    ...
    lines.push(`   匹配依据：来源标题《${clean(h.title, 60)}》${h.demo ? '（演示样例）' : ''}${h.hits > 1 ? `（本次检索中命中 ${h.hits} 次）` : ''}`)
...
export async function answer(message, session = {}) {
  ...
  if (intent.isFollowUp) {
    ...
    const query = `${h.fullName || h.name}${campusAsked ? ' ' + campusAsked : ''} 地址 官方预约 挂号入口`
    let res
    try {
  ...
  if (hospitals.length) {
    ...
    out.push(renderHospitalCards(hospitalsIn, { city: intent.cities[0] || session.city || '', topic, queriedAt }))
