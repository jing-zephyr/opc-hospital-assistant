...
export const SAY = {
  ...
  MEDICAL: [
    '抱歉，这个问题我帮不上您——我不是医生，**不做诊断、治疗方案或用药指导**，万一说错了会耽误您。',
    '',
    '**建议您这样就医**：',
    '① 带上既往病历资料，到正规医院咨询专业医务人员（不知道去哪家，可以问我「北京有哪些医院设有心血管内科」这类问题）；',
    '② 想先电话咨询，可以打医院官方总机或科室电话——告诉我**城市和医院名**，我帮您查该院官网公布的联系方式与预约入口；',
    '③ 症状紧急请立即拨打 **120**。',
  ].join('\n'),
...
export function extractHospitals(items, opts = {}) {
  ...
  // 用户点名了某家医院 → 该院卡片钉在最前：最终排序此前只看 trustedEntry，
  // 并列时按搜索返回顺序走，实测"那地坛医院怎么挂号"把**天坛**顶到了第一位（相邻名串场）。
  const wantHit = (h) => {
    if (!wantHospital) return 0
    for (const n of [h.fullName, h.name]) {
      const s = String(n || '')
      if (s && (s.includes(wantHospital) || wantHospital.includes(s))) return 1
    }
    return 0
  }
  const ordered = [...final].sort((a, b) => (wantHit(b) - wantHit(a)) || ((trustedEntry(b) ? 1 : 0) - (trustedEntry(a) ? 1 : 0)))
