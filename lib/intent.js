...
const APPOINTMENT_RE = /(预约|挂号|官方入口|怎么挂|咨询电话|联系电话|电话|联系方式)/
// 真机实测补充：导航/地图/科室分布 类问法（"有地图可以导航吗""这个科室是怎么分布的我要怎么找"）
const ADDRESS_RE = /(地址|在哪|怎么走|怎么去|怎么找|导航|地图|分布|在几楼|几层|地铁|院区|分院|位置)/
...
export function parseIntent(message, session) {
  ...
  let ordinal = parseOrdinal(text)
  // ⭐ 模糊承接（真机实测修复）：没说"第 N 家"、本轮也没重复科室/资源/院名，
  //    但带了地址/预约类信号（"有地图可以导航吗""怎么挂号""电话多少"），且上一轮有医院列表
  //    → 默认承接第 1 家，并在回答里显式说明（可改口"第 2 家"）。
  //    不修这个的话，这类问法会拿"地图/导航"当检索词重搜一遍，必然"未查到"，且后续连续全挂。
  const fuzzyFollowUp = ordinal === 0
    && (flags.address || flags.appointment)
    && departments.length === 0 && resources.length === 0 && !hospitalQuery
    && Array.isArray(sess.hospitals) && sess.hospitals.length > 0
  if (fuzzyFollowUp) ordinal = 1
  ...
  return {
    ...
    ordinal,
    fuzzyFollowUp,                                    // true = 未点名第几家，默认承接第 1 家（回答里需显式说明）
    specificDate,
    hospitalQuery,
