...
export function loadSecrets() {
  ...
  // 多 key 轮换支持：逗号分隔（"k1,k2"）→ 某把额度耗尽/失效时自动换下一把；单 key 写法完全兼容
  out.BOCHA_API_KEYS = String(out.BOCHA_API_KEY || '').split(',').map((s) => s.trim()).filter(Boolean)
  out.TAVILY_API_KEYS = String(out.TAVILY_API_KEY || '').split(',').map((s) => s.trim()).filter(Boolean)
  cache = out
  return cache
}
...
export function channelStatus() {
  const s = loadSecrets()
  return {
    bocha: Boolean(s.BOCHA_API_KEY), tavily: Boolean(s.TAVILY_API_KEY),
    bochaKeys: (s.BOCHA_API_KEYS || []).length, tavilyKeys: (s.TAVILY_API_KEYS || []).length,
    bochaDeadKeys: KEY_DEAD.bocha.size, tavilyDeadKeys: KEY_DEAD.tavily.size,
  }
}

// ---------- 多 key 轮换（评审期额度保障，2026-09-26 甲方确认双通道各自加号） ----------
// 401/403/429/432 = 这把 key 的额度/鉴权问题 → 标记失效并自动换下一把；网络异常不标记（可能只是抖动）。
// ⚠️ 纪律：只记"不可用"状态码，绝不留存/回显 key 本体。
const KEY_DEAD = { bocha: new Map(), tavily: new Map() }   // key → 失效时的 HTTP 状态码
function rotationNote(keys, dead) {
  return keys.length > 1 ? `（已配 ${keys.length} 把密钥轮换，${dead.size} 把失效）` : ''
}
...
export async function searchBocha(query, count = 8) {
  ...
  const keys = loadSecrets().BOCHA_API_KEYS || []
  if (!keys.length) return []
  const dead = KEY_DEAD.bocha
  const alive = keys.filter((k) => !dead.has(k))
  if (!alive.length) { CHANNEL_STATUS.bocha = '额度/鉴权失效' + rotationNote(keys, dead); return [] }
  let lastErr = '网络异常'
  for (const key of alive) {
    try {
      ...
      if (!r.ok) {
        // 额度/鉴权类故障：标记这把 key 失效，换下一把；其余故障码不轮换（换了也一样）
        if (r.status === 401 || r.status === 403 || r.status === 429) {
          dead.set(key, r.status)
          STATS.keyRotations++
          lastErr = r.status === 403 ? 'HTTP 403（额度/套餐不足）'
            : r.status === 401 ? 'HTTP 401（密钥无效）' : 'HTTP 429（请求过频）'
          continue
        }
        CHANNEL_STATUS.bocha = `HTTP ${r.status}` + rotationNote(keys, dead)
        return []
      }
    ...
    } catch { lastErr = '网络异常'; continue }   // 网络抖动不标记 key，试下一把
  }
  CHANNEL_STATUS.bocha = lastErr + rotationNote(keys, dead)
  return []
}
...
export async function searchTavily(query, count = 8) {
  ...
  const keys = loadSecrets().TAVILY_API_KEYS || []
  if (!keys.length) return []
  const dead = KEY_DEAD.tavily
  const alive = keys.filter((k) => !dead.has(k))
  if (!alive.length) { CHANNEL_STATUS.tavily = '用量超限/鉴权失效' + rotationNote(keys, dead); return [] }
  let lastErr = '网络异常'
  for (const key of alive) {
    try {
      ...
      if (!r.ok) {
        // 用量/鉴权类故障：标记这把 key 失效，换下一把；其余故障码不轮换
        if (r.status === 401 || r.status === 403 || r.status === 429 || r.status === 432) {
          dead.set(key, r.status)
          STATS.keyRotations++
          lastErr = r.status === 432 || r.status === 429 ? `HTTP ${r.status}（用量超限/过频）`
            : r.status === 401 ? 'HTTP 401（密钥无效）' : 'HTTP 403（额度/套餐不足）'
          continue
        }
        CHANNEL_STATUS.tavily = `HTTP ${r.status}` + rotationNote(keys, dead)
        return []
      }
  ...
  CHANNEL_STATUS.tavily = lastErr + rotationNote(keys, dead)
  return []
}
...
const CACHE = new Map()
// 同一问题重复问直接走缓存（省额度、也更快）。本地默认 10 分钟保鲜；
// 评审期在 Netlify 配 OPC_CACHE_TTL_MIN=1440 拉到 24 小时——评委反复问相似问题不重复烧额度。
// ⚠️ 缓存内容仍是**真实检索结果**（响应带 cached/cachedAt 标注），不是预制数据。
const CACHE_TTL_MS = Math.max(1, Number(process.env.OPC_CACHE_TTL_MIN || 10)) * 60 * 1000
const MAX_CACHE = 200

export const STATS = {
  searches: 0, cacheHits: 0, retries: 0, failures: 0, keyRotations: 0,
