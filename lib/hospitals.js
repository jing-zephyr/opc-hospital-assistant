...
export const OFFICIAL_SITES = {
  ...
  'tsinghua.edu.cn': {
    // ⚠️ 这是**院校**（主管部门/院校类权威来源），不是医院 —— kind=authority 让它参与 A 级定级，
    //    但**不会**被当成"医院条目"展示；它证明的是 hospital 字段指向的那家医院（实测曾把"清华大学"当医院卡片列出）。
    kind: 'authority',
  ...
  'bjdth.com.cn': {
    name: '首都医科大学附属北京地坛医院',
    city: '北京',
    note: '官网首页可访问（<title>引导页-首都医科大学附属北京地坛医院，2026-09-25 fetch 核实）；市属公立三级甲等（传染病专科）',
    evidence: 'https://www.bjdth.com.cn/',
    categoryEvidence: 'https://www.bjdth.com.cn/',
  },
  'bch.com.cn': {
    name: '首都医科大学附属北京儿童医院',
    city: '北京',
...
export function officialOf(host) {
  const h = String(host || '').toLowerCase().replace(/^www\./, '')
  // 条目可自带 kind（如 tsinghua.edu.cn 是**院校**权威来源而非医院，避免被当成医院条目展示）
  if (OFFICIAL_SITES[h]) return { ...OFFICIAL_SITES[h], host: h, kind: OFFICIAL_SITES[h].kind || 'hospital-official' }
  if (AUTHORITY_SITES[h]) return { name: AUTHORITY_SITES[h], host: h, kind: 'authority' }
  return null
}

// ---------- 常见简称/口语 → 已核实全称（只收**无歧义**的；每个目标名都在 OFFICIAL_SITES / KNOWN_HOSPITALS 里可核） ----------
// 用途：用户说"北大人民医院怎么挂号"时，检索词换成官方全称，命中率与来源级别都会显著提升（实测："那北大人民医院"直接搜几乎无 A/B 命中）。
// ⚠️ 纪律：这是**查表归一**，不是编造——别名指向的全称必须在白名单/已知院名表里存在；查不到就原样返回。
export const HOSPITAL_ALIASES = {
  '协和': '北京协和医院',
  '北大人民医院': '北京大学人民医院',
  '北大第一医院': '北京大学第一医院',
  '北大医院': '北京大学第一医院',
...
/** 剥口语前缀：句首的"那/这个/那家"等指代词不是院名的一部分（实测："那北大人民医院"直接进了检索词） */
export function stripNamePrefix(name) {
  return String(name || '').replace(/^[那这该](?=[\u4e00-\u9fa5])/, '')
}

/** 把口语/简称/带前缀的院名归一到**已核实全称**；归一不了就返回原串（不编）。 */
export function canonicalHospitalName(name) {
  let n = stripNamePrefix(String(name || '').trim())
  if (!n) return n
  if (HOSPITAL_ALIASES[n]) return HOSPITAL_ALIASES[n]
  // 部分包含： "北大人民医院" 已在别名表； "北京协和医院"本身即全称
  for (const [alias, full] of Object.entries(HOSPITAL_ALIASES)) {
    if (alias.length >= 2 && n.includes(alias) && !n.includes(full)) {
      // ⚠️ 别名后面还拖着"另一个完整院名"尾巴的，是**独立医疗机构**，不归并
      //   （实测回归："北京中医医院顺义医院"被别名"北京中医医院"吃成母院 —— 顺义医院是独立机构）
      const rest = n.replace(alias, '')
      if (/(医院|卫生院|保健院|门诊部|医学中心|医疗中心)/.test(rest) && rest.length > 2) continue
      return full
    }
  }
  // 白名单/已知院名的包含匹配（"北京协和医院（西单院区）"这类尾巴由调用方另行处理）
  const bare = n.replace(/[（(][^)）]*[)）]\s*$/, '')
  // ⚠️ "长名 ⊃ 已核实名 + 机构尾巴" 是**独立机构**，不能归并（"北京积水潭医院郑州医院"≠"北京积水潭医院"）
  const hasOwnTail = (target) => {
    const rest = bare.replace(target, '')
    return /(医院|卫生院|保健院|门诊部|医学中心|医疗中心|院区|分院)/.test(rest) && rest.length > 2
  }
  for (const v of Object.values(OFFICIAL_SITES)) {
    const target = (v.hospital || v.name || '').replace(/[（(][^)）]*[)）]\s*$/, '')
    if (target && (target === bare || (bare.includes(target) && !hasOwnTail(target)) || target.includes(bare))) return v.hospital || v.name
  }
  const kh = KNOWN_HOSPITALS.find((k) => k === bare || (bare.includes(k) && !hasOwnTail(k)) || k.includes(bare))
  return kh || n
}

/** 由院名反查**白名单官网域名**（该院自己的官网，不是院校/集团页）。查不到返回 null（不编域名）。 */
export function officialSiteByName(name) {
  const canon = canonicalHospitalName(name)
  if (!canon) return null
  const bare = canon.replace(/[（(][^)）]*[)）]\s*$/, '')
  for (const [host, v] of Object.entries(OFFICIAL_SITES)) {
    if ((v.kind || 'hospital-official') === 'authority') continue   // 院校/主管部门页不是"该院官网"
    const target = (v.hospital || v.name || '').replace(/[（(][^)）]*[)）]\s*$/, '')
    if (target && (target === bare || bare.includes(target) || target.includes(bare))) {
      return { host, name: v.hospital || v.name, city: v.city || '', evidence: v.evidence || '' }
    }
  }
  return null
}
