...
export function canonicalHospitalName(name) {
  ...
  for (const [alias, full] of Object.entries(HOSPITAL_ALIASES)) {
    if (alias.length >= 2 && n.includes(alias) && !n.includes(full)) return full
  }
  ...
  for (const v of Object.values(OFFICIAL_SITES)) {
    ...
    if (target && (target === bare || bare.includes(target) || target.includes(bare))) return v.hospital || v.name
  }
  const kh = KNOWN_HOSPITALS.find((k) => k === bare || bare.includes(k) || k.includes(bare))
  return kh || n
}
