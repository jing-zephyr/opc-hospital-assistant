...
log('| 检查项 | 结果 |')
log('|---|---|')
log('| 手机端适配（viewport + 媒体查询） | ' + (/viewport-fit=cover/.test(idx) && /@media \(max-width:560px\)/.test(idx) ? '✅' : '❌') + '（代码级证据见《进阶项实测证据_手机适配.md》；**视觉截图待补**） |')
...
log('| **医院结果对比表**：含「所在地区 / 院区 / 公开资源与服务信息」三列 | ' + (/<th>所在地区<\/th>/.test(idx) && /<th>院区<\/th>/.test(idx) && /<th>公开资源与服务信息<\/th>/.test(idx) ? '✅' : '❌') + ' |')
log('| 「来源级别」已降为**辅助列**（表头标注"辅助"） | ' + (/来源级别<br><span[^>]*>（辅助）/.test(idx) ? '✅' : '❌') + ' |')
log('| 对比仍**最多 3 家** | ' + (/CMP\.size >= 3/.test(idx) ? '✅' : '❌') + ' |')
log('| 对比**不以医疗质量排名替代**（页面明文保留） | ' + (/不以缺乏依据的医疗质量排名替代/.test(idx) ? '✅' : '❌') + ' |')
log('')
...
log('| 检查项 | 结果 |')
log('|---|---|')
log('| 网页版视角切换器（公众/导医/管理） | ' + (/id="views"/.test(idx) && /data-view="staff"/.test(idx) ? '✅' : '❌') + ' |')
log('| 小程序版视角切换器 | ' + (/id="viewbar"/.test(mini) ? '✅' : '❌') + ' |')
log('| 导医视角：**待人工核实来源清单**（C 级 + 原文核对链接） | ' + (/待人工核实来源/.test(idx) ? '✅' : '❌') + ' |')
log('| 管理视角：**运行指标面板**（/api/stats） | ' + (/运行指标/.test(idx) ? '✅' : '❌') + ' |')
log('| 页面明确标注"无需登录 · 不采集身份" | ' + (/无需登录 · 不采集身份/.test(idx) ? '✅' : '❌') + ' |')
...
log('| 进阶4 小程序实际集成 | ❌ 未做（赛题标注非强制） | 需在运营小程序授权；已用 `/mini.html` 模拟调用方代替并标注 |')
log('')
