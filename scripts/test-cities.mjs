...
for (const region of outside) {
  ...
  const checks = isProvince
    ? [
        // 判据口径修正（2026-09-24）：省级输入原本要求 `status === 'ok'`。
        //   `status` 是由**本次检索到的来源级别**推导的（只有 C 级线索时必须是 partial —— 不许把
        //   "非权威来源"说成"已核实"），与"省级是否被识别"无关。上游检索结果会随抓取时间变化：
        //   本轮「河北省」命中的 16 条全部是 C 级（医院官网不在本系统已核实的官网白名单里 → 按规则只能算线索），
        //   于是 status=partial —— 这是**如实**结果，不是省级识别失败。
        //   因此与 A 组采用**同一标准**（ok/partial 均可），并保留 A 组的"不得无理由降级"硬约束：
        //   只要有 A/B 级来源，status 就不许是 partial（**没有放水**）。
        ['status=ok/partial（按省检索并如实给出候选城市；不因来源级别误判为"没识别出地区"）', j.status === 'ok' || j.status === 'partial'],
        ['有 A/B 级来源时不得降级为 partial（与 A 组同一硬约束）', !((j.sources || []).some((s) => s.tier === 'A' || s.tier === 'B') && j.status === 'partial')],
